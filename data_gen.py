...
def compute_reward_from_step(step_info, my_targets):
    ...
    score += len(own) * 50
    score -= len(enemy) * 20
    if score == 0 and not step_info.get('WHITE_BALL_INTO_POCKET', False) and not step_info.get('BLACK_BALL_INTO_POCKET', False) and not step_info.get('FOUL_FIRST_HIT', False) and not step_info.get('NO_POCKET_NO_RAIL', False):
        score = 10
    return float(score)
...
def run_games(n_games, target_cycle, seed):
    ...
    for i in range(n_games):
        ...
        while True:
            ...
            states.append(s)
            actions.append(action_norm(a, load_meta(None, len(s))))
            rewards.append(r)
